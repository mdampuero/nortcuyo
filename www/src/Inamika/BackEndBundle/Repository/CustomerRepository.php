...
class CustomerRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function search($query=null,$limit=0,$offset=0,$sort=null,$direction=null){
        ...
        if($limit==0) $limit=30;
        $qb= $this->getAll()
        ->join('e.customerCategory','customerCategory')
        ...
        if($query){
            ...
            if(count($words)>1){
                foreach ($words as $key => $word) {
                    $queryString=array();
                    $queryString[]="CONCAT(e.email,e.name,e.document,customerCategory.name) LIKE :word".$key;
                    $qb->setParameter('word'.$key,"%".$word."%");
                    $qb->andWhere(join(' AND ',$queryString));
                }
            }else{
                $qb->andWhere("CONCAT(e.email,e.name,e.document,customerCategory.name) LIKE :query")->setParameter('query',"%".$query."%");
            }
        }
        return $qb;
    }
